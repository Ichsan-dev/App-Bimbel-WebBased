...
@section('Content')
    ...
    <div class="content">
          <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form action="{{ route('SectionUpdate', $section->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card-body">
                  <div class="form-group">
                    <label for="exampleInputEmail1">Title :</label>
                    <input type="Text" class="form-control" id="title" placeholder="Enter Title" name="vtitle" value="{{ $section->title }}">
                    @error('vtitle')
                         <small>{{$message}}</small>
                    @enderror
                  </div>
                    <div class="form-group">
                        ...
                        <textarea name="vcontent" id="content" class="form-control">{{ $section->content }}</textarea>
                      @error('vcontent')
                         <small>{{$message}}</small>
                    @enderror
                      </div>
                      <div class="form-group">
                        ...
                        <img src="{{ asset($section->thumbnail) }}" alt="Current Image" style="max-width: 200px;">
                    </div>
                </div>
                <!-- /.card-body -->
