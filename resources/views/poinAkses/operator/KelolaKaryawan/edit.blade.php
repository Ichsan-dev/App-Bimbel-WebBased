...
@section('Content')
    ...
    <div class="content">
      ...
      <div class="card card-success">
          ...
          <!-- /.card-header -->
        <div class="card-body">
            <form action="{{route('KaryawanUpdate', $datakaryawan->id)}}" method="POST" enctype="multipart/form-data"  id="formKaryawan"> 
              @csrf
              @method('PUT')
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  <label>Nama Lengkap</label>
                    <input type="text" class="form-control" id="nama" placeholder="" name="vnama" value="{{$datakaryawan->nama}}">
                      @error('vnama')
                        <small>{{ $message }}</small>
                      @enderror
                </div>
                ...
                <div class="form-group">
                  <label>Jabatan :</label>
                    <select name="jabatan_id" class="form-control" id="jabatan_id">
                      ...
                      <option value="{{$datakaryawan->jabatan_id}}">{{$datakaryawan->jabatan->nama_jabatan}}</option>
                ...
                <div class="form-group">
                    ...
                    <div class="form-check" style="display: inline-block; margin-left: 10px; margin-right:10px">
                        <input class="form-check-input" type="radio" name="vjenis_kelamin" id="pria" value="Laki-laki" {{$datakaryawan->jk == 'Laki-laki' ? 'checked' : ''}}>
                        <label for="pria">Laki-laki</label>
                    </div>
                    <div class="form-check" style="display: inline-block; margin-right: 30px;">
                        <input class="form-check-input" type="radio" name="vjenis_kelamin" id="wanita" value="Perempuan" {{$datakaryawan->jk == 'Perempuan' ? 'checked' : ''}}>
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" id="no_telp" placeholder="Cth: 0877xxx" name="vno_telp" value="{{$datakaryawan->no_telp}}">
                    @error('vnohp')
                      <small>{{ $message }}</small>
                    @enderror
                </div>
                <div class="form-group">
                  <label for="exampleInputEmail">Email</label>
                    <input type="text" class="form-control" id="email" placeholder="Cth: 123@abc.com" name="vemail" value="{{$datakaryawan->email}}">
                      @error('vemail')
                        <small>{{ $message }}</small>
                      @enderror
                </div>
                ...
                <div class="form-group">
                  <label for="exampleInputPend_Akhir">Pendidikan Terakhir</label>
                    <select class="form-control" name="vpend_akhir">
                      ...
                      <option value="SD" {{$datakaryawan->pend_akhir == 'SD' ? 'selected' : ''}}>SD</option>
                      <option value="SMP" {{$datakaryawan->pend_akhir == 'SMP' ? 'selected' : ''}}>SMP</option>
                      <option value="SMA/SMK" {{$datakaryawan->pend_akhir == 'SMA/SMK' ? 'selected' : ''}}>SMA/SMK</option>
                      <option value="Strata-1" {{$datakaryawan->pend_akhir == 'Strata-1' ? 'selected' : ''}}>Strata-1</option>
                      <option value="Diploma IV" {{$datakaryawan->pend_akhir == 'Diploma IV' ? 'selected' : ''}}>Diploma IV</option>
                    </select>
                </div>
                <!-- /.form-group -->
                <div class="form-group">
                  <label for="tanggal_lahir">Tanggal Lahir:</label>
                  <input type="date" name="vtgl" class="form-control" id="vtgl" value="{{$datakaryawan->tanggal_lahir}}">
                </div>
                <!-- /.form-group -->
                <div class="form-group">
                  <label for="exampleInputKomentar">Alamat</label>
                    <textarea name="valamat" id="alamat" class="form-control">{{$datakaryawan->alamat}}</textarea>
                      @error('valamat')
                        <small>{{ $message }}</small>
                      @enderror
                </div>
